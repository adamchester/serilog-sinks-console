...
namespace Serilog.Sinks.SystemConsole.Output
{
    ...
    class ThemedExceptionTokenRenderer : OutputTemplateTokenRenderer
    {
        ...
        public ThemedExceptionTokenRenderer(ConsoleTheme theme, PropertyToken pt)
        {
            _theme = theme;
            _pt = pt;
        }
        ...
        public override void Render(LogEvent logEvent, TextWriter output)
        {
            ...
            {
                Debugging.SelfLog.WriteLine(
                    "Exception while writing themed exception: {0}", ex.ToString());

                var fallback = new ExceptionTokenRenderer(_theme, _pt);
                fallback.Render(logEvent, output);
            }
